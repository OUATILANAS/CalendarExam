...
import com.example.demo.common.SecurityParams;
...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter{
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        try {
            Personne myUser = new ObjectMapper().readValue(request.getInputStream(), Personne.class);
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        Personne user = (Personne) authResult.getPrincipal();

        PersonneService userService = CalendarApplication.getCtx().getBean(PersonneService.class);

        Personne myUser = userService.findByUsernameWithRoles(user.getUsername());
